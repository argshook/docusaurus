...
:::warning Feed generation [RSS feeds](#feed) require the author's email to be set for the author to appear in the feed. :::
...
:::warning Don't forget to delete the existing homepage at `./src/pages/index.js` or else there will be two files mapping to the same route! :::
